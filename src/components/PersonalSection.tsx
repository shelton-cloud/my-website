...
export function PersonalSection() {
    ...
    return (
        <section id="personal" className="mb-20 scroll-mt-24">
            <FadeIn>
            ...
            {/* Instrument Rack */}
            <div className="mb-12 grid grid-cols-1 sm:grid-cols-3 gap-1">
                {instruments.map((inst, i) => (
                    <FadeIn key={inst.name} delay={0.2 + (i * 0.1)}>
                        <div className="bg-surface-container-low border border-outline/10 p-6 hardware-hover group">
                            <span className="text-[10px] font-mono text-primary/60 uppercase tracking-[0.2em] mb-2 block">{inst.type}</span>
                            <h4 className="headline-md mb-1 group-hover:text-primary transition-colors">{inst.name.toUpperCase()}</h4>
                            <p className="text-xs text-outline tracking-wider uppercase font-medium">{inst.detail}</p>
                        </div>
                    </FadeIn>
                ))}
            </div>
            ...
            <div className="space-y-6">
                ...
                <div className="flex flex-wrap gap-2">
                    {profile.personal.hobbies.map((hobby, index) => (
                        <span 
                            key={index} 
                            className="px-4 py-1.5 border border-outline/20 text-on-surface rounded-sm label-sm bg-surface-container-low"
                        >
                            {hobby}
                        </span>
                    ))}
